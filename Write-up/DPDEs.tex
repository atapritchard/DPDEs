...
\begin{abstract}
Standard methods for solving Partial Differential Equations (PDEs) include Euler steps, Runge-Kutta, Galerkin methods, and others. These methods mesh the domain in which we want to solve the PDE and then take small steps to solve the PDE. However, the combination of an exponentially increasing number of both grid points and steps and decreasing numerical stability makes this process infeasible for high-dimensional problems. In this paper we attempt to solve the Heat Equation, a well-studied PDE, using the Deep Galerkin Method, which uses deep neural networks to approximate a solution to the PDE via the Universal Approximation Theorem.
\end{abstract}
